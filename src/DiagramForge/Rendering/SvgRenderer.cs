...
public sealed class SvgRenderer : ISvgRenderer
{
    ...
    public string Render(Diagram diagram, Theme theme)
    {
        ...
        // Background
        sb.AppendLine($"""  <rect width="{width}" height="{height}" fill="{theme.BackgroundColor}" rx="{theme.BorderRadius}" ry="{theme.BorderRadius}"/>""");

        // Title
        if (!string.IsNullOrWhiteSpace(diagram.Title))
        {
            sb.AppendLine($"""  <text x="{width / 2}" y="{theme.DiagramPadding - 4}" text-anchor="middle" font-family="{theme.FontFamily}" font-size="{theme.TitleFontSize}" font-weight="bold" fill="{theme.TextColor}">{Escape(diagram.Title)}</text>""");
        }
    ...
    private static void AppendDefs(StringBuilder sb, Theme theme)
    {
        ...
        sb.AppendLine($"""      <polygon points="0 0, 10 3.5, 0 7" fill="{theme.EdgeColor}"/>""");
    ...
    private static void AppendNode(StringBuilder sb, Node node, Theme theme)
    {
        ...
        string stroke = node.StrokeColor ?? theme.NodeStrokeColor;
        ...
        switch (node.Shape)
        {
            ...
            case Shape.Ellipse:
                ...
                sb.AppendLine($"""    <ellipse cx="{F(cx)}" cy="{F(cy)}" rx="{F(cx)}" ry="{F(cy)}" fill="{fill}" stroke="{stroke}" stroke-width="{theme.StrokeWidth}"/>""");
                break;

            case Shape.Diamond:
                double mx = node.Width / 2, my = node.Height / 2;
                sb.AppendLine($"""    <polygon points="{F(mx)},0 {F(node.Width)},{F(my)} {F(mx)},{F(node.Height)} 0,{F(my)}" fill="{fill}" stroke="{stroke}" stroke-width="{theme.StrokeWidth}"/>""");
                break;

            case Shape.Pill:
            case Shape.Stadium:
                sb.AppendLine($"""    <rect width="{F(node.Width)}" height="{F(node.Height)}" rx="{F(node.Height / 2)}" ry="{F(node.Height / 2)}" fill="{fill}" stroke="{stroke}" stroke-width="{theme.StrokeWidth}"/>""");
                break;

            default: // Rectangle / RoundedRectangle
                sb.AppendLine($"""    <rect width="{F(node.Width)}" height="{F(node.Height)}" rx="{F(rx)}" ry="{F(rx)}" fill="{fill}" stroke="{stroke}" stroke-width="{theme.StrokeWidth}"/>""");
                break;
        }
        ...
        string textColor = node.Label.Color ?? theme.TextColor;
        double fontSize = node.Label.FontSize ?? theme.FontSize;

        sb.AppendLine($"""    <text x="{F(textX)}" y="{F(textY)}" text-anchor="middle" font-family="{theme.FontFamily}" font-size="{fontSize}" fill="{textColor}">{Escape(node.Label.Text)}</text>""");
        sb.AppendLine("  </g>");
    }

    private static void AppendEdge(StringBuilder sb, Edge edge, Node source, Node target, Theme theme)
    {
        // Draw a straight line from the bottom-center of source to the top-center of target
        ...
        double cpOffset = Math.Abs(y2 - y1) * 0.4;
        string cp1 = $"{F(x1)},{F(y1 + cpOffset)}";
        string cp2 = $"{F(x2)},{F(y2 - cpOffset)}";

        string strokeColor = edge.Color ?? theme.EdgeColor;
        ...
        sb.AppendLine($"""  <path d="M {F(x1)},{F(y1)} C {cp1} {cp2} {F(x2)},{F(y2)}" fill="none" stroke="{strokeColor}" stroke-width="{theme.StrokeWidth}"{strokeDash}{markerEnd}/>""");
        ...
        if (edge.Label is not null && !string.IsNullOrWhiteSpace(edge.Label.Text))
        {
            double lx = (x1 + x2) / 2;
            double ly = (y1 + y2) / 2 - 4;
            sb.AppendLine($"""  <text x="{F(lx)}" y="{F(ly)}" text-anchor="middle" font-family="{theme.FontFamily}" font-size="{theme.FontSize * 0.85}" fill="{theme.SubtleTextColor}" font-style="italic">{Escape(edge.Label.Text)}</text>""");
        }
    }

    private static void AppendGroup(StringBuilder sb, Group group, Theme theme)
    {
        string fill = group.FillColor ?? "#F3F4F6";
        string stroke = group.StrokeColor ?? "#D1D5DB";

        sb.AppendLine($"""  <rect x="{F(group.X)}" y="{F(group.Y)}" width="{F(group.Width)}" height="{F(group.Height)}" rx="{theme.BorderRadius}" ry="{theme.BorderRadius}" fill="{fill}" stroke="{stroke}" stroke-width="{theme.StrokeWidth}" opacity="0.6"/>""");

        if (!string.IsNullOrWhiteSpace(group.Label.Text))
        {
            sb.AppendLine($"""  <text x="{F(group.X + 8)}" y="{F(group.Y + theme.FontSize + 4)}" font-family="{theme.FontFamily}" font-size="{theme.FontSize * 0.9}" fill="{theme.SubtleTextColor}" font-weight="bold">{Escape(group.Label.Text)}</text>""");
        }
    }
